...
export type Synthesis = {
  ...
  dissent: { from: string; point: string }[];
  minority: string;
  open: string[];
  evidence: number;
  counterfactuals: { name: string; impact: string }[];
};
...
export const SYNTHESIS: Synthesis = {
  consensus:
    "A sustained PRC-imposed customs quarantine of Taiwanese ports is the modal next escalation rung within a 12-month window (probability 0.42, 7-persona aggregate). Japan is structurally non-neutral by geography and treaty interpretation; the financial-shock vector will lead and likely dominate the kinetic vector by 24–72 hours. US deterrence credibility is contingent on a publicly visible, time-compressed munitions flow that markets will read as confirmation of escalation — creating a credibility-versus-stability dilemma with no clean exit.",
  confidence: 0.78,
  consensusFrom: ["analyst", "indopacom", "tsmc", "ishiba"],
  dissent: [
    {
      from: "xi",
      point: "Frames any escalation as lawful internal customs enforcement; rejects 'quarantine' framing entirely.",
    },
    {
      from: "lai",
      point: "Treats any blockade or quarantine as casus belli; rejects probabilistic framing as legitimizing.",
    },
    {
      from: "citizen",
      point: "Rejects all escalation pathways; weights status-quo preservation above legitimacy or deterrence credibility.",
    },
  ],
  minority:
    "If PLA Rocket Force interdicts Bashi Channel within first 72 hours of trigger, US escort posture pivots to submarine-delivered resupply, extending useful munitions delivery window to 14–21 days (Adm. Paparo, single source).",
  open: [
